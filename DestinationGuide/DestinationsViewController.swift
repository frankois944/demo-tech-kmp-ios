...
class DestinationsViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    let destinationService: DestinationFetchingService = DestinationFetchingServiceImpl()
    let historyService: DestinationHistoryService = DestinationHistoryServiceImpl()
    ...
    override func viewDidLoad() {
        ...
        destinationService.getDestinations { destinations in
            switch onEnum(of: destinations) {
            case .failure:
                break
            case let .success(data):
                self.array = data.destinations.sorted(by: { $0.name < $1.name })
            }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        historyService.addToHistory(destination: desti)
        
        destinationService.getDestinationDetails(destinationId: desti.id) { result in
            DispatchQueue.main.async {
                switch onEnum(of: result) {
                case let .failure(data):
                    let alert = UIAlertController(title: "Erreur", message: data.error.name, preferredStyle: .alert)
                    alert.addAction(UIAlertAction(title: "Annuler", style: .cancel))
                    self.present(alert, animated: true)
                case let .success(data):
                    self.navigationController?.pushViewController(DestinationDetailsController(title: data.destinationDetails.name,
                                                                                               webviewUrl: URL(string: data.destinationDetails.url)!), animated: true)
                }
        }
    }
}
